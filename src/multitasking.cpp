...
namespace multitasking
{
    ...
    uint64_t execution_index = MAX_PROCESS_NUMBER;
    uint64_t process_count = 0;
    uint64_t max_process_index = 0;
    ...
    uint64_t create_process(void* entrypoint,paging::page_table_t* paging_root,interrupt::privilege_level_t level)
    {
        if(process_count<MAX_PROCESS_NUMBER)
        {
            ...
            process_array[process_id].is_ready = true;
    ...
    {
        if(process_array[id].is_present&&id!=0)
        {
    ...
    void scheduler()
    {
        ...
        {
            if(scheduler_timer_ticks>=timesharing_interval||(!process_array[execution_index].is_present)||(!process_array[execution_index].is_ready))
            {
                scheduler_timer_ticks=0;
                do
                {
                    execution_index++;
                    execution_index%=MAX_PROCESS_NUMBER;
                    execution_index%=max_process_index+1;
                } while (!(process_array[execution_index].is_present&&process_array[execution_index].is_ready));
            }
        }
        else
        {
    ...
    void panic()
    {
        ...
        printf("Cause: %s (0x%x)\nError found at: 0x%p\n",interrupt::isr_messages[context->int_num],context->int_info,context->rip);
